//! `zig build test-linux`: builds every test executable that runs under Linux, and the io_uring
//! probe, for the container target, and installs them under zig-out/linux/. It runs none of them:
//! tools/linux_test.sh runs them in Docker. build.zig stays short (CLAUDE.md, Layout), so the
...
//!
//! The step writes two files beside the executables, and tools/linux_test.sh reads both:
...
pub fn add(b: *std.Build, optimize: std.builtin.OptimizeMode) void {
    ...
    const step = b.step(
        "test-linux",
        "Build the Linux test executables and the io_uring probe for tools/linux_test.sh",
    );
