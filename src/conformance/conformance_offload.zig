...
//!
//! The two backends are asked the same thing and answer differently. On kqueue a file operation
//! blocks the loop thread, so the policy decides what happens: `refuse` ends the operation with
//! `unsupported`, `blocking` performs it inline, and `offload` hands it to the caller's thread. On
//! io_uring the kernel completes it without a thread, so the option is taken and ignored and every
//! policy reads the block. `backend.files_block` tells the two apart, and each scenario asserts what
//! its own backend should do.
//!
//! The `offload` scenario runs a real thread, because the policy exists so a thread that owns no
//! loop can hand a result back. A synchronous stand-in would pass with the cross-thread path
//! broken.
...
///
/// It is the smallest thing that is still a real hand-off. A caller with a pool puts a queue here
/// instead, and rotor sees no difference.
const Pool = struct {
    ...
    /// same event and the same bytes.
    served: std.atomic.Value(u32),
    /// While set, the worker holds the work instead of running it. A scenario sets it, acts while
    /// the operation is out on the thread, then clears it.
    ...
    fn submit(context: ?*anyopaque, work: *Work) void {
        ...
        // Each scenario collects one operation's event before submitting the next, so the slot
        // is free. A scenario that broke that would drop an operation, so this halts instead.
        std.debug.assert(previous == null);
    }

    /// The worker thread. It owns no loop.
...
const Fixture = struct {
    ...
    /// The rings one worker needs. The backend that holds rings reports the size; the other holds
    /// none, and Zig has no zero-length array here, so the floor is one byte.
...
test "the refuse policy ends a file read with unsupported, where a file blocks the loop" {
    ...
    if (backend.files_block) {
        // Decision 18's default: a caller who asked for nothing is told, not stalled.
...
test "the blocking policy performs a file read inline, on both backends" {
    ...
    const event = try loop_fixture.read_block(&into);
    try testing.expectEqual(@as(u32, block_bytes), try event.outcome());
    // The file was sized and never written, so it reads as zeros. The buffer started as 0xaa, so
    // the changed bytes show the read happened.
...
test "the offload policy completes a file read on the caller's own thread" {
    ...
    try testing.expectEqual(@as(u32, block_bytes), try event.outcome());
    try testing.expectEqualSlices(u8, &@as([block_bytes]u8, @splat(0)), &into);
    // One final event and no more: decision 5, rule 1, across the thread boundary.
...
test "an offloaded read of every block completes, so the ring is drained and reused" {
    ...
    // one message would pass the scenario above and fail here.
...
test "an offloaded result lands in the tick that waited for it, not the one after" {
    ...
    // and the drain after the wait turns the result into an event. A tick that drained only before
    // its wait would return 0 here and make the caller tick again.
...
test "an offloaded read that is cancelled still ends with exactly one final event" {
    ...
    // already running cannot be stopped, so a cancel means "end it when it returns" (decision 18's
    // open question 2, and decision 5 for an operation the kernel owns).
    ...
    // `canceled` depends on which won the race; that there is exactly one is decision 5, rule 1.
    var round: u32 = 0;
    var produced: u32 = 0;
...
test "the offload policy leaves a socket operation's cancel alone" {
    ...
    // block the loop. Its cancel stays synchronous and ends with `canceled` at once. A loop that
    // treated every operation under this policy as offloaded would wait for a worker that was given
    // nothing, and this accept would never end.
...
test "the io_uring backend takes the policy and nothing changes" {
    ...
    // caller can set without an offload both read the block and neither refuses.
