//! The synchronous socket calls on the uring backend: `linux_shared`'s, with this backend's socket
...
/// The type flags every socket this backend opens carries: it closes on exec, and it blocks,
/// because the ring does the waiting.
pub const socket_flags: u32 = linux.SOCK.CLOEXEC;
