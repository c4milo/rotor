...
pub const Remote = struct {
    ...
    ///
    /// It is the same two steps `kqueue_submit.post` makes for a loop, with this remote's id as the
    /// sender. A target that names no running loop, including another remote, is `LoopNotFound`; a
    /// target whose ring from this sender is full is `MailboxFull`.
    pub fn post(remote: *Remote, target: core.LoopId, message: core.Message) PostError!void {
        ...
        // Negative covers both a loop that is not running and another remote, which publishes a
        // sentinel below zero for exactly this reason.
        if (target_queue < 0) return error.LoopNotFound;
        if (!registry.mailbox(remote.id, target).push(message)) return error.MailboxFull;
        // The same handshake a loop makes: the target said it would sleep, so it has to be woken, and
        // `settle_to_sleep` re-reads the rings so a message pushed before it saw the flag is found
        // anyway (decision 12, point 6).
