...
//!
//! This is `src/uring/uring_sync_socket.zig` with one change: **every socket is opened with
//! SOCK_NONBLOCK.** io_uring does the waiting, so the uring backend's sockets block. This backend
//! makes each transfer itself when epoll reports readiness (decision 20), so a socket that blocked
//! would block the loop. Linux takes the flag in `socket(2)` itself, so nothing is set afterwards,
//! where macOS needs three calls per socket (`kqueue_sync_socket.zig`). The file is a copy and not
//! a shared one because the graph has no module the two Linux backends share
//! (`epoll_address.zig` says why).
//!
...
//!
//! `close_now` and `descriptor_of` live here and `epoll_sync_file.zig` imports them, because a
//! file needs both and a socket needed them first.
...
/// block, because the loop makes each call itself and waits for readiness instead.
pub const socket_flags: u32 = linux.SOCK.CLOEXEC | linux.SOCK.NONBLOCK;

/// A TCP socket of `family`, closed on exec and non-blocking.
pub fn open_socket(family: Address.Family) SocketError!Descriptor {
    const domain: u32 = switch (family) {
        .ipv4 => linux.AF.INET,
        .ipv6 => linux.AF.INET6,
    };
    const rc = linux.socket(domain, linux.SOCK.STREAM | socket_flags, linux.IPPROTO.TCP);
...
/// A UDP socket of `family`, closed on exec and non-blocking, bound to `address` when one is
/// given. A port of 0 takes one the kernel chooses, which `local_address` reports. Closes the
...
pub fn open_datagram(
    ...
    const rc = linux.socket(domain, linux.SOCK.DGRAM | socket_flags, linux.IPPROTO.UDP);
    const errno = linux.errno(rc);
    if (errno != .SUCCESS) return socket_error(errno);
    const socket = descriptor_of(rc);
    errdefer close_now(socket);
    @import("epoll_datagram.zig").apply_options(socket, family, options);
    if (bind_to) |address| {
...
/// listener's backlog, so a blocking connect returns with no accept. The client is opened here
/// and not with `open_socket`, whose socket does not block and would answer EINPROGRESS: the
/// listener is what these tests are about, and a blocking client keeps them free of a wait.
fn connect_to(address: *const Address) !Descriptor {
    const domain: u32 = switch (address.family) {
        .ipv4 => linux.AF.INET,
        .ipv6 => linux.AF.INET6,
    };
    const opened = linux.socket(domain, linux.SOCK.STREAM | linux.SOCK.CLOEXEC, linux.IPPROTO.TCP);
    try testing.expectEqual(E.SUCCESS, linux.errno(opened));
    const client = descriptor_of(opened);
...
test "a socket closes on exec, does not block, and takes TCP_NODELAY; others are refused" {
    if (builtin.os.tag != .linux) return error.SkipZigTest;
    const socket = try open_socket(.ipv4);
    // The one difference from the uring backend's socket, and the one the loop depends on.
...
test "a datagram socket closes on exec, does not block, and a bound one has taken a port" {
    if (builtin.os.tag != .linux) return error.SkipZigTest;
    const any_port = Address.ipv4(.{ 127, 0, 0, 1 }, 0);
    const bound = try open_datagram(.ipv4, &any_port, .{});
    defer close_now(bound);
    try testing.expect((try open_flags(bound)).NONBLOCK);
