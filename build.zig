//! Build graph for rotor: `zig build` compiles every module, `zig build lint` scores every
//! function's cognitive complexity and runs the rules of tools/lint over the tree, `zig build
//! test` runs the lint and then every module's unit tests, and `zig build test-<module>` runs one
//! module's tests with nothing else in the graph, which is what a mutation is measured against.
//!
...
pub fn build(b: *std.Build) void {
    ...
    // Assertions stay on in production (CLAUDE.md non-negotiable 1), so the build offers Debug
    ...
    // docs/decisions/0008-hot-path-assertions.md states which assertions the hot path carries.
    const optimize = b.standardOptimizeOption(.{ .preferred_optimize_mode = .ReleaseSafe });
    assert(optimize == .Debug or optimize == .ReleaseSafe);

    const graph = modules.add(b, target, optimize);

    // Everything below is rotor's own build: the tests, the checks and the tools. A project that
    // depends on rotor stops here, before the tools request pepegrillo.
...
/// A host module that imports `pepegrillo`, compiled in Debug: every tool, and nothing else.
fn tool_module(
    b: *std.Build,
...
/// branch adds. `zig build install-commit-lint` installs the linter alone, which
/// .githooks/pre-push runs when zig-out/bin/commit_lint is missing.
fn add_commit_lint_step(
    ...
    const install_tool_step = b.step(
        "install-commit-lint",
        "Install the commit-message linter alone",
    );
