...
/// The other work on the machine while each candidate's runs were taken, one window per candidate.
...
fn collect(
    ...
    while (round < options.rounds) : (round += 1) {
        for (candidates, 0..) |candidate, index| {
            ...
            // spoiled the 2026-09-20 attempts, and only a reading on each side sees it.
