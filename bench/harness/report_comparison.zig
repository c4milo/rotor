...
pub const Losses = struct {
    ...
    pub fn any(losses: Losses) bool {
        return losses.throughput or losses.p50 or losses.p99 or losses.p999;
    }
};
...
test "a comparison marks every row rotor loses, in the first column, and counts them" {
    ...
    const results = [_]Result{
        echo_result("libuv", "1.0.0", 100_000, .{ 20_000, 40_000, 29_000 }),
        rotor_result,
        echo_result("libxev", "2.0.0", 250_000, .{ 9_000, 25_000, 28_000 }),
        echo_result("std.Io.Threaded", "0.16.0", 200_000, .{ 10_000, 20_000, 30_000 }),
        echo_result("std.Io.Uring", "0.16.0", 150_000, .{ 10_000, 19_999, 30_000 }),
    };
    ...
    try testing.expectEqualStrings("Workload `echo`: 4 cores, 1024 connections, " ++
        ...
        "| p50 ns | p99 ns | p999 ns | overflow |\n" ++
        "|---|---|---|---:|---:|---:|---:|---:|---:|\n" ++
        "| **ROTOR LOSES: p999** | libuv | 1.0.0 | 100000 | 500 | 20000 | 40000 | 29000 | 0 |\n" ++
        "| baseline | rotor | 0.1.0 | 200000 | 1000 | 10000 | 20000 | 30000 | 0 |\n" ++
        "| **ROTOR LOSES: throughput, p50, p999** | libxev | 2.0.0 | 250000 | 1250 " ++
        "| 9000 | 25000 | 28000 | 0 |\n" ++
        "| no loss | std.Io.Threaded | 0.16.0 | 200000 | 1000 | 10000 | 20000 | 30000 | 0 |\n" ++
...
test "a comparison rotor loses no row of says so in plain letters, and the mark is absent" {
    const results = [_]Result{
        rotor_result,
        echo_result("libuv", "1.0.0", 100_000, .{ 20_000, 40_000, 60_000 }),
    };
    var buffer: [1024]u8 = undefined;
    var writer: Writer = .fixed(&buffer);
    try render_comparison(&writer, &results);
    try testing.expect(std.mem.endsWith(u8, writer.buffered(), "| no loss | libuv | 1.0.0 " ++
        "| 100000 | 500 | 20000 | 40000 | 60000 | 0 |\n" ++
