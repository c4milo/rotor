...
/// Where a probe's thread runs: the harness's placement, which the echo and datagram programs
/// share, so a row here and a row of theirs mean the same thing by "one core". A caller that gets
/// anything but `.pinned` from `pin_current_thread` must report it: the two ends may have shared
/// a core, which is the thing a two-core row separates.
const harness_placement = @import("harness").placement;
pub const Placement = harness_placement.Placement;
pub const place_current_thread = harness_placement.place_current_thread;
pub const pin_current_thread = harness_placement.pin_current_thread;
pub const first_cpu = harness_placement.first_cpu;
pub const second_cpu = harness_placement.second_cpu;
