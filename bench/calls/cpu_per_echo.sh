...
#
# Run it in a container that may use io_uring, with the echo programs built for it at /b:
#
...
measure() {
  ...
  awk -v label="$label" -v payload="$payload" -v echoes="${echoes:-0}" \
    ...
    -v seconds="$SECONDS_PER_RUN" 'BEGIN {
      if (echoes == 0) { printf "| %s | %s | no echoes | | | | |\n", payload, label; exit }
      ticks = user + kernel
      printf "| %s | %s | %d | %.2f | %.1f | %.1f | %.3f |\n", payload, label, echoes,
...
echo
echo "| payload | server | echoes | cpu µs per echo | busy % | in the kernel % | switches per echo |"
echo "|---:|---|---:|---:|---:|---:|---:|"
